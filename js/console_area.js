...
$('#tabla_area').on('click','.eliminar',function(){
    ...
    Swal.fire({
        title: '¿Eliminar el área '+data.area_nombre+'?',
        text: "Los usuarios serán reasignados a Mesa de Partes. No se elimina si tiene trámites históricos vinculados.",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                type:'POST',
                data:{ id: data.area_cod }
            }).done(function(resp){
                if(resp==1){
                    Swal.fire("Mensaje de Confirmación","Área eliminada correctamente","success").then(()=>{
                        tbl_area.ajax.reload();
                    });
                }else if(resp==3){
                    Swal.fire("Mensaje de Advertencia","No se puede eliminar: el área tiene trámites históricos registrados como origen o destino","warning");
                }else if(resp==4){
                    Swal.fire("Mensaje de Advertencia","No se puede eliminar el área principal (Mesa de Partes)","warning");
                }else{
                    Swal.fire("Mensaje de Error","No se pudo eliminar el área","error");
                }
            });
        }
    });
