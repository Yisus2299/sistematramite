...
$('#tabla_usuario').on('click','.eliminar',function(){
    ...
    Swal.fire({
        title: '¿Eliminar al usuario '+data.usu_usuario+'?',
        text: "Se eliminará permanentemente. El historial de movimientos se conservará sin referencia al usuario.",
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                if(resp==1){
                    Swal.fire("Mensaje de Confirmación","Usuario eliminado correctamente","success").then(()=>{
                        tbl_usuario.ajax.reload();
                    });
                }else if(resp==5){
                    Swal.fire("Mensaje de Advertencia","No puede eliminar su propio usuario mientras tiene la sesión activa","warning");
                }else{
                    Swal.fire("Mensaje de Error","No se pudo eliminar el usuario","error");
                }
            });
        }
    });
